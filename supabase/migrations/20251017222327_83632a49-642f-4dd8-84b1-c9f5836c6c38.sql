-- Atualizar função get_mood_metrics para corrigir mapeamento de mood e adicionar contagem de votos por equipe
CREATE OR REPLACE FUNCTION public.get_mood_metrics(days_period integer DEFAULT 30, limit_records integer DEFAULT 50, offset_records integer DEFAULT 0)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Calculate average mood (valores corretos do banco: otimo, bem, neutro, cansado, dificil)
  SELECT AVG(
    CASE mood
      WHEN 'otimo' THEN 5
      WHEN 'bem' THEN 4
  ...
  -- Count difficult moods (valores corretos)
  ...
  -- Get team moods COM CONTAGEM DE VOTOS
  SELECT json_agg(
    json_build_object(
      'team', team,
